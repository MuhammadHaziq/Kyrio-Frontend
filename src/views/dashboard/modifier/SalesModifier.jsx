...
const SalesModifier = () => {
  ...
  const sale_modifier_summary = useSelector(
    (state) => state.reports.salesModifierReducer.sale_modifier_summary
  );
  const loadingFilter = useSelector(
    (state) => state.reports.salesModifierReducer.loading
  );
        ...
        return itm.options.map((op) => {
          exortData.push({
            ...
            ["Net Sales"]: parseFloat(
              op.grossSales - op.refundQuantitySold
            ).toFixed(2),
            ["H"]: "",
          });
        });
            ...
            <CCardHeader>
              <CRow>
                <CCol xs="12" sm="6" md="6" xl="xl" className="mb-3 mb-xl-0">
                  {typeof sale_modifier_summary !== "undefined" &&
                  sale_modifier_summary.length > 0 ? (
                    <CSVLink
                      data={exportData}
                      filename={
                        "SalesByModifier" + dateformat(new Date()) + ".csv"
                      }
                      target="_blank"
                    >
