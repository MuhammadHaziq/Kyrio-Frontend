  ...
  return (dispatch) => {
    try {
      axios({
        ...
        .then((response) => {
          dispatch({
            type: GET_SALES_CATEGORY_SUMMARY,
...
export const toggle_sales_category_summary_single_select = (data) => {
  return (dispatch) => {
    dispatch({
      type: TOGGLE_SALE_CATEGORY_SUMMARY_SINGLE_SELECT,
      response: data,
    });
  };
};
...
export const toggle_sales_category_summary_all_select = (status) => {
  return (dispatch) => {
    dispatch({
      type: TOGGLE_SALE_CATEGORY_SUMMARY_ALL_SELECT,
      response: status,
    });
  };
};
...
export const delete_sales_category_summary = (ids) => {
  return (dispatch) => {
    dispatch({
      type: DELETE_SALES_CATEGORY_SUMMARY,
    ...
    //     })
    //         .then((response) => {
    //             dispatch({ type: DELETE_SALES_CATEGORY_SUMMARY, response: JSON.parse(ids) });
    //             let msg = {
    //                 open: true,
    //                 message:
...
export const update_row_data = (row) => {
  return (dispatch) => {
    dispatch({ type: ROW_DATA_SALES_CATEGORY_SUMMARY, response: row });
  };
};
