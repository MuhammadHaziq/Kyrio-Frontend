...
const AddItem = (props) => {
  ...
  const [compositeItem, setCompositeItems] = useState([]);
  ...
  useEffect(() => {
    if (item.item_list !== undefined && item.item_list.length > 0) {
      const compositeItems = item.item_list.map((item) => {
        return {
          value: item._id,
          label: item.name,
        };
  ...
  console.log("compositeItem", compositeItem);
  return (
    <React.Fragment>
      <CCard>
        <CCardBody>
