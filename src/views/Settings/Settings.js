...
import { toggleSettingSideBar } from "../../actions/settingsAction";
...
const Settings = () => {
  ...
  const modules = useSelector(
    (state) => state.auth.user.roleData.allowBackoffice.modules
  );
  const roleTitle = useSelector((state) => state.auth.user.roleData.title);

  const [settingCk, setSettingCk] = useState(
    modules.filter(
      (mod) =>
        mod.backoffice.isMenu &&
        mod.backoffice.isChild &&
        mod.enable &&
        mod.backoffice.title !== "Manage POS devices"
    ).length > 0
  );
  const [editSettingCk, setEditSettingCk] = useState(
    modules.filter(
      (mod) =>
        mod.backoffice.isMenu &&
        !mod.backoffice.isChild &&
        mod.enable &&
        mod.backoffice.title === "Edit general settings"
    ).length > 0
  );
  ...
  useEffect(() => {
    ...
    if (!mobileCheck()) {
      dispatch(toggleSettingSideBar(true));
    }
    if (mobileCheck()) {
      dispatch(toggleSettingSideBar(false));
    }
  }, [dispatch]);
  const setting = [];
  useEffect(() => {
    if (settings !== undefined && settings) {
      ...
      let settingCheck = modules.filter(
        (mod) => mod.backoffice.isMenu && mod.backoffice.isChild && mod.enable
      );
      let editSettingCheck = modules.filter(
        (mod) =>
          mod.backoffice.isMenu &&
          !mod.backoffice.isChild &&
          mod.enable &&
      ...
      if (settingCheck.length > 0 || editSettingCheck.length > 0) {
        ...
        if (!mngPaymentTypes) {
          settings.find(
            (st) => st.module.title === "Payment types"
          ).enable = false;
        } else {
          settings.find(
            (st) => st.module.title === "Payment types"
        ...
        if (!mngDiningOptions) {
          settings.find(
            (st) => st.module.title === "Dining options"
          ).enable = false;
        } else {
          if (
            settings.find((st) => st.module.title === "Dining options").enable
          ) {
            settings.find(
              (st) => st.module.title === "Dining options"
            ).enable = true;
        ...
        if (!mngKitchenPrinters) {
          settings.find(
            (st) => st.module.title === "Kitchen printers"
          ).enable = false;
        } else {
          if (
            settings.find((st) => st.module.title === "Kitchen printers").enable
          ) {
            settings.find(
              (st) => st.module.title === "Kitchen printers"
            ).enable = true;
        ...
        if (!mngPOSDevices) {
          settings.find(
            (st) => st.module.title === "POS devices"
          ).enable = false;
        } else {
          settings.find(
            (st) => st.module.title === "POS devices"
          ...
          <CCol xs="12" sm="12" md="12" lg="4">
            <CListGroup id="list-tab" role="tablist">
              <CSidebar
                ...
                fixed={false}
                unfoldable
                onShowChange={(val) => dispatch(toggleSettingSideBar(val))}
              >
