    ...
    <CDataTable
      ...
      scopedSlots={{
        ...
        net_sales: (item) => (
          <td>
            <strong>
              {<Amount value={item.grossSales - item.refundGrossSales} />}
            </strong>
          </td>
        ),
        details: (item, index) => {
            ...
            >
              <tbody>
                {item.options.map((op) => {
                  return (
                    <tr>
                      {fields.find((item) => item.key === "Modifier")
                        ?.isShow ? (
                        <td
                          style={{
                            width: fieldLength + "%",
                      ...
                      {fields.find((item) => item.key === "quantitySold")
                        ?.isShow ? (
                        <td style={{ width: fieldLength + "%" }}>
                          {op.quantitySold}
                        </td>
                      ) : (
                        ""
                      )}
                      {fields.find((item) => item.key === "grossSales")
                        ?.isShow ? (
                        <td style={{ width: "16%" }}>
                          {<Amount value={op.grossSales} />}
                        </td>
                      ) : (
                        ""
                      )}
                      {fields.find((item) => item.key === "refundQuantitySold")
                        ?.isShow ? (
                        <td style={{ width: fieldLength + "%" }}>
                          {op.refundQuantitySold}
                        </td>
                      ) : (
                        ""
                      )}
                      {fields.find((item) => item.key === "refundGrossSales")
                        ?.isShow ? (
                        <td style={{ width: fieldLength + "%" }}>
                          {<Amount value={op.refundGrossSales} />}
                        </td>
                      ) : (
                        ""
                      )}
                      {fields.find((item) => item.key === "net_sales")
                        ?.isShow ? (
                        <td style={{ width: fieldLength + "%" }}>
                          {
                            <Amount
                              value={op.grossSales - op.refundGrossSales}
                            />
                          }
                        </td>
                      ) : (
